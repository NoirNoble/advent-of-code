...
func getPowerOfGame(cGame cube_game) int {
	rounds := cGame.rounds
	var max_red int
	var max_green int
	var max_blue int
	...
	for i := 0; i < len(rounds); i++ {
		if rounds[i].red > max_red {
			max_red = rounds[i].red
		}

		if rounds[i].green > max_green {
			max_green = rounds[i].green
		}

		if rounds[i].blue > max_blue {
			max_blue = rounds[i].blue
		}
